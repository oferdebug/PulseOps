...
export function useFileUpload(entityType: AttachmentEntityType, entityId: string) {
    ...
    async function uploadFile(file: File): Promise<Attachment | null> {
        ...
        const entry: FileUploadProgress = { file, progress: 0, status: 'uploading' };
        ...
        try {
            ...
            setUploads(prev =>
                prev.map((u, i) =>
                    i === index ? { ...u, progress: 100, status: 'success', attachmentId: attachment.id } : u
                )
            );
